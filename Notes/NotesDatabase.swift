...
class NotesDatabase: NotesDatabaseProtocol {
    ...
    @MainActor
    func getNote(identifier: UUID) throws -> Note? {
        do {
            
            var predicate = #Predicate<Note> { object in
            ...
            var object = try container.mainContext.fetch(descriptor)
            return object.first
        } catch {
            print("Error \(error.localizedDescription)")
            throw DatabaseError.errorFetch
    ...
    func update(identifier: UUID, title: String, text: String?) throws {
        ...
        do {
            ...
            print("Error actualizandop información")
            throw DatabaseError.errorUpdate
        }
    }
